...
- -e 显示不可见字符，并在结尾显示$符号
  ...
  - display the non-displaying characters, and display a dollar sign ('$') at the end of each line.
...
- -n 查看后n行 tail -n 5 .bashrc 显示.bashrc的后5行
...
uniq用于删除重复的行，需要先排序 sort fiLeName | uniq
...
tr [a-z] [A-Z] 将小写转化为大写，前者为被替换，后面的为替换后的内容

删除 tr --delete "source" < fileName 
...
- -o：只显示匹配PATTERN部分
- A[数字n] 列出符合PATTERN模式行的前n行
- B[数字n] 列出符合PATTERN模式行的后n行
- C[数字n] 列出符合PATTERN模式行的前后各n行
