...
1. [软件包管理](#软件包管理)
    1. [CentOS系统上rpm命令管理程序包](#centos系统上rpm命令管理程序包)
        1. [安装](#安装)
        2. [查询](#查询)
        3. [升级](#升级)
        4. [卸载](#卸载)
    ...
    2. [yum](#yum)
        ...
        18. [yum命令行选项](#yum命令行选项)
        19. [yum的repo配置文件中可用的变量](#yum的repo配置文件中可用的变量)
        20. [创建yum仓库](#创建yum仓库)
    3. [程序包编译安装](#程序包编译安装)
        1. [项目管理工具](#项目管理工具)
...
- dpt包管理器（debian package tools），debian开发，deb包
- rpm包管理器Redhat Package Manager --> RPM is Package Manager，redhat,最初由perl语言开发，后由C重写，rpm包
- Gentoo的包管理器
- Archlinux的包管理器
...
- -i: rpm -qi PACKAGE_NAME: 查询指定包的说明信息；
- -l: rpm -ql PACKAGE_NAME: 查询指定包安装后生成的文件列表；
- -c: rpm -qc PACEAGE_NEME：查询指定包安装的配置文件；
- -d: rpm -qd PACKAGE_NAME: 查询指定包的配置文件；
