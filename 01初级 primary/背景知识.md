...
5. [运维硬件基础](#运维硬件基础)
    ...
    5. [硬盘](#硬盘)
        1. [企业应用:](#企业应用)
        2. [DELL阵列卡（RAID卡，企业必备）](#dell阵列卡raid卡企业必备)
        3. [SSD最好，都选SSD吧？](#ssd最好都选ssd吧)
        4. [运维三大件: CPU 硬盘 内存](#运维三大件-cpu-硬盘-内存)
    6. [光驱：](#光驱)
    7. [优化网站、服务器的关键:](#优化网站服务器的关键)
    8. [CPU架构类型](#cpu架构类型)
6. [操作系统基础知识](#操作系统基础知识)
    1. [Unix简介](#unix简介)
    2. [UNIX主要发行版本](#unix主要发行版本)
    3. [GNU工程](#gnu工程)
7. [Linux内核](#linux内核)
    1. [内核功能](#内核功能)
    2. [主要发行版本](#主要发行版本)
...
### 硬盘
生产环境：常用3.5寸机械硬盘；sas为主，其次sata，sas和sata接口兼 容。
...
#### 企业应用:
1. 常规正式场景选SAS银盘（转速15000转/分，机械磁盘转速高性能好）
...
RAID0没有数据冗余，但是访问速度最快。

RAID1镜像卷
#### SSD最好，都选SSD吧？
淘宝网企业案例：服务器会把sata和ssd结合起来，热点存储，程序动态调度。
#### 运维三大件: CPU 硬盘 内存
...
企业案例：
1.极端方法——用于门户非核心，大并发直接写到内存，再按一定频率写入磁盘。特点：快；可能丢data
2.常规方法——用于中小企业，并发不是很大的网站先把数据写入disk，再通过程序从disk读到内存，访问时直接读内存。特点：低性能；data不丢
